...
export const getIntegerOrDefault = (value?: unknown, defaultValue = NaN): number => {
  const valueNumber = Number(value)
...
export const getIntegerOrUndefined = (value?: unknown, defaultValue = NaN) => {
  const iord = getIntegerOrDefault(value, defaultValue)
  return (iord === defaultValue || (Number.isNaN(iord) && Number.isNaN(defaultValue)))
    ? undefined
    : iord
}
